/*
A minimum spanning tree is a special kind of tree that minimizes the lengths or "weights" of tree edges.  An example is a cable
company that wants to lay line to multiple neighborhoods; by minimizing the amount of cable laid, the cable company will save money.
A tree has one path that joins any two vertices.
...
void printMST(int T[][V-2], int G[V][V]){
    cout << "Minimum spanning tree edges with cost:\n" << endl;
    int sum {0};
    for (int i {0}; i < V - 2; i++){
        int c = G[T[0][i]][T[1][i]];
